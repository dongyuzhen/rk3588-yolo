...
ThreadPoll::ThreadPoll(const char* model_path, int num_threads, int img_w, int img_h)
{
    // 这里可以做一些通用初始化，比如 run_flag=true
    run_flag = true;
    init(model_path, num_threads, img_w, img_h);
}
...
void ThreadPoll::init(const char* model_path, int num_threads, int img_w, int img_h)
{

    if(num_threads <= 0) num_threads = 1; // 保底
    for(int i = 0; i < num_threads; i++)
    {
        auto yolo = std::make_shared<Yolov5s>(model_path, i % 3, img_w, img_h);
